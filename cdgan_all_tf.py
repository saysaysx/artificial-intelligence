...
def train_descriminator(real_img,labels):
    with tf.GradientTape() as tape:
        d = descriminator([real_img,labels],training = True)
        loss_real = - tf.reduce_mean(tf.math.log(d+1.0e-12))
        rx =  tf.random.normal(shape=(n_batch, num_hide))
        fake_image = generator([rx,labels])
        d = descriminator([fake_image,labels],training = True)
        loss_fake = - tf.reduce_mean(tf.math.log(1-d+1.0e-12))
