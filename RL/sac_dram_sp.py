...
class sac:
    def __init__(self,env):
        ...
        lay3 = Dense(400, activation="linear") (lay)
        layp3 = Dense(self.len_act, activation="softmax") (lay3)


        self.modelp = keras.Model(inputs=inp1, outputs=[layp1, layp2, layp3])
    ...
    def get_net_act(self,l_state):
        ...
        indv = int(random.random()*2.9999999)
    ...
    def train_q1(self, inp, inp_next, actn, rew, dones):
        with tf.GradientTape(persistent=True) as tape1:
            ...
            minq2 = tf.minimum(targ[1] , targ[2])
            minq3 = tf.minimum(targ[0] , targ[2])


            dift1 = tf.reduce_sum((minq1-tf.stop_gradient(self.alphav)*logpi[0])*y_pi[0], axis=-1)
            dift2 = tf.reduce_sum((minq2-tf.stop_gradient(self.alphav)*logpi[1])*y_pi[1], axis=-1)
            dift3 = tf.reduce_sum((minq3-tf.stop_gradient(self.alphav)*logpi[2])*y_pi[2], axis=-1)

            qvt = []
            qvt.append(tf.stop_gradient(rew+self.gamma*dift1*(1-dones)))
            qvt.append(tf.stop_gradient(rew+self.gamma*dift2*(1-dones)))
            qvt.append(tf.stop_gradient(rew+self.gamma*dift3*(1-dones)))
            ...
            lossq = tf.reduce_mean(dif[0]+dif[1]+dif[2])


            trainable_varsa = self.modelq[0].trainable_variables+self.modelq[1].trainable_variables+self.modelq[2].trainable_variables
    ...
    def train_actor1(self, inp):
        with tf.GradientTape() as tape2:
            ...
            y_pii1 = self.modelp(inp1, training = True)
            y_pii = y_pii1
            y_pii = tf.clip_by_value(y_pii,1e-15,0.99999999999999)

            logpi = tf.math.log(y_pii)
            entr = - tf.reduce_mean(tf.reduce_sum(y_pii[0]*logpi, axis=-1))
            ypi_border = tf.reduce_mean(tf.math.exp(-y_pii/0.0001))*1000.0


            minq1 = tf.minimum(qv[0], qv[1])
            minq2 = tf.minimum(qv[1], qv[2])
            minq3 = tf.minimum(qv[0], qv[2])


            diflm1 = tf.reduce_sum(y_pii[0]*(tf.stop_gradient(self.alphav)*logpi[0] - minq1),axis=-1)
            diflm2 = tf.reduce_sum(y_pii[1]*(tf.stop_gradient(self.alphav)*logpi[1] - minq2),axis=-1)
            diflm3 = tf.reduce_sum(y_pii[2]*(tf.stop_gradient(self.alphav)*logpi[2] - minq3),axis=-1)


            dm = tf.reduce_mean(diflm1+diflm2+diflm3)


            lossp = dm + ypi_border#+ tf.square(0.01-dify)*1000.0#+kb*0.0001 #+ tf.nn.relu(self.entrmax*0.01-entr)*10 #+ corrdisp*maxq*0.05 #+ tf.reduce_mean(val)*maxq*0.02 #+ tf.reduce_mean(entr)*maxq*0.0005
